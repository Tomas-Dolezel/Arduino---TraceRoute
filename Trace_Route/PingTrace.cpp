 ...
 */
// traceroute ability added Tomas Dolezel <skilledtom@seznam.cz>
...
void ICMPPing::operator()(const IPAddress& addr, int nRetries, int reqttl, ICMPEchoReply& result)
{
    ...
    {
        result.status = sendEchoRequest(addr, echoReq, reqttl);
...
ICMPEchoReply ICMPPing::operator()(const IPAddress& addr, int nRetries, int reqttl)
{
    ICMPEchoReply reply;
    operator()(addr, nRetries, reqttl, reply);
    return reply;
}

Status ICMPPing::sendEchoRequest(const IPAddress& addr, const ICMPEcho& echoReq, int reqttl)
{
    ...
    W5100.writeSnTTL(_socket, reqttl);
