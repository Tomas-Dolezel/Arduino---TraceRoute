...
public:
    ...
    */
    ICMPEchoReply operator()(const IPAddress&, int nRetries, int reqttl);
    ...
    */
    void operator()(const IPAddress& addr, int nRetries, int reqttl, ICMPEchoReply& result);

private:

    Status sendEchoRequest(const IPAddress& addr, const ICMPEcho& echoReq, int reqttl);
